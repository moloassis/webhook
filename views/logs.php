...
<div class="container">
    ...
    <div style="display: flex; justify-content: space-between; align-items: center; padding-bottom: 0.8rem; border-bottom: 1px solid var(--border-color); flex-wrap: wrap; gap: 1rem;">
        ...
        <div>
            <?php if (isTenantReadOnlyMode()): ?>
                <button type="button" class="btn-premium" style="background: rgba(255, 255, 255, 0.05); border-color: rgba(255, 255, 255, 0.1); color: rgba(255, 255, 255, 0.3); font-weight: 600; width: auto; font-size: 0.8rem; padding: 0.5rem 1rem; cursor: not-allowed;" disabled title="Ação desabilitada em modo de inspeção">
                    🗑️ Limpar Histórico
                </button>
            <?php else: ?>
    ...
    <?php if (isTenantReadOnlyMode()): ?>
        ...
        <div style="background: rgba(255, 165, 0, 0.1); border: 1px solid rgba(255, 165, 0, 0.3); padding: 1rem; border-radius: 12px; color: #ffa502; font-weight: 500; font-size: 0.9rem; margin-top: 1.5rem; display: flex; align-items: center; gap: 10px; width: 100%;">
            <span>⚠️</span>
            <span><strong>Modo de Inspeção (Somente Leitura):</strong> Você está visualizando os logs desta organização como Superadmin. Ações de limpeza estão desabilitadas.</span>
        </div>
    <?php endif; ?>
